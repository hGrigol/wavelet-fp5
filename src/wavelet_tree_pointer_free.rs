...
impl<T> WaveletTreePointerFree<T> where T: Hash+Clone+Ord+Debug+Copy {
  ...
  pub fn create<S:Clone+Iterator<Item=T>>(sequence: S) -> WaveletTreePointerFree<T>{
	...
	println!("{:?}",alphabet_og);
	WaveletTreePointerFree{alphabet : alphabet_og, bitmap :bitmap,wordlength : wordlength}
  }
	...
	if min == max{
		...
		if !self.bitmap.get(index as u64) {
			...
			if left == 0 {
				...
				return self.access_bitmap(next_index,min,compute_breakpoint(&min,&max),left+ self.wordlength,left + self.wordlength+self.bitmap.rank_0(right as u64).unwrap() as usize -1)
			}
			//nicht erste ebene
			else{
				//println!("gehe links");
				...
				let new_right = left-1+self.wordlength+self.bitmap.rank_0(right as u64).unwrap() as usize - self.bitmap.rank_0(left as u64-1).unwrap()  as usize;
				return self.access_bitmap(next_index,min,compute_breakpoint(&min,&max),new_left,new_right);
			}
		...
		// falls wir nach rechts gehen
			...
			if left == 0 {
				...
				let  next_index = left +  self.wordlength + self.bitmap.rank_0(right as u64).unwrap() as usize-1 +self.bitmap.rank_1(index  as u64).unwrap() as usize;

				return self.access_bitmap(next_index,compute_breakpoint(&min,&max)+1,max,left+ self.wordlength+self.bitmap.rank_0(right as u64).unwrap() as usize,right+self.wordlength)}
			//nicht erste ebene
			else{
				//println!("gehe rechts");
				//println!("linker index: {}",left);
				//println!("rechter index: {}",right);
				let next_index = left + self.wordlength  as usize + self.bitmap.rank_0(right as u64).unwrap() as usize - self.bitmap.rank_0(left as u64-1).unwrap() as usize+(self.bitmap.rank_1(index as u64).unwrap() as usize - self.bitmap.rank_1(left as u64-1).unwrap() as usize)-1;
				let new_right = right+&self.wordlength;
				let new_left= left+self.wordlength+self.bitmap.rank_0(right as u64).unwrap() as usize-self.bitmap.rank_0(left as u64-1).unwrap() as usize;
				return self.access_bitmap(next_index,compute_breakpoint(&min,&max)+1,max,new_left,new_right);
			}	
		}
